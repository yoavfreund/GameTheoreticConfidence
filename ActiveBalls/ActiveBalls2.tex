...
\[\Pr_{\samp \sim p^{n}}\Bigl[\exists {B\in\B}:
  p_{\samp}(B)>2k/n \mbox{ and } p(B) < k/n \Bigr] \leq \delta,
\]
and
\[\Pr_{\samp \sim p^{n}}\Bigl[\exists {B\in\B}:
  p_{\samp}(B)< 2k/n \mbox{ and } p(B) > 4k/n \Bigr] \leq \delta.
\]
\end{theorem}
...
E_1 = \text{"}\exists {B\in\B}:
  p_{\samp_1}(B)>2k/n \mbox{ and } p(B) < k/n
\text{"}, 
\]
and let $E_2$ denote the event:
\[
E_2 = \text{"}\exists {B\in\B}:
  p_{\samp_1}(B)>2k/n \mbox{ and } p_{\samp_2}(B) < 1.5k/n
\text{"}.
\]
...
Let $B\in\B$ such that $p_{\samp_1}(B)>2k/n \mbox{ and } p(B) < k/n$.
By the definition of $E_2$
it suffices to show that 
\[
\Pr\Bigl[ p_{\samp_2}(B) \geq (3/2)p(B)\Bigr]
\leq
9/10
\]
(since $p(B) < k/n$).
