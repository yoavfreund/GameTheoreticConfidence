...
We only need to consider $B$'s such that $k(B) > 2k$.
Fix such a $B$.
...
Note that $B$ causes $E_2$ to occur if and only if $X\leq 3k(B)/7$.
To upper bound the probability of this event we use
Mcdiarmid's inequality~\cite{}.
To this end note that $X$ is a function of $X_1,\ldots,X_{k(B)}$
with $\mu:=\Ex[X]=k(B)/2$, and that $X$ satisfies the bounded
difference condition with $c_i=1$ (because it is a sum of indicators). 
Therefore, Mcdiarmid's inequality implies
that
\[\Pr\bigl[X\leq3k(B)/7\bigr]=\Pr\bigl[X-\mu \leq -k(B)/14\bigr]\leq\exp\bigl(-2(1/14)^2k(B)\bigr) < \exp(-k/50) \]
(where the last inequality is because $k(B) > 2k$).
Now, since the number of distinct restrictions $B|_S$ is at most $(2n)^d$, by the union bound it follows that
...
%{\bf High level.} $p(B)\geq k_0/n$ implies 
%that with a constant probability $k_2(B)$ is sufficiently large,
%say at least $k_0/100$.
%This should imply that the $\bias_{\samp_2}(B)$ is sufficiently close
%to $\bias(B)$, say to an additive factor of $O\bigl(\sqrt{\frac{d\log n + \log(1/\delta)}{k_2(B)}}\bigr)$.
%Consequently, this should imply that $\bias_{\samp_2}(B)$ and $\bias_{\samp_1}(B)$
%are at least some $\Omega\bigl(\sqrt{\frac{d\log n + \log(1/\delta)}{k_1(B)}}\bigr)$ with a constant probability,
%which is $E_1$ (or qualitatively equivalent to it). To conclude, conditioned on $E_2$, the probability of $E_1$
