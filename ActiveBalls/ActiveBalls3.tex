...
$\constr{\cB}{+}{\sample{S}},\constr{\cB}{-}{\sample{S}}$ and
$\constr{\cB}{0}{\sample{S}}$, we partition the instances in
$\universe{X}$ into {\em interior} and a {\em boundary} sets. Roughly
speaking, interior points are those on which all of the specialists
agree. To this set of points we add points where all of the
specialists with small support agree. \yoav{I am planning to create a
  jupyter notebook and generate some figures that will be added here,
  to aid intuition.}
...
\item {\em Effective Boundary} contains all points that are not in
  either interior set:
  \[
  \nonconform(\universe{Z},\cB^{-1},\cB^{+1},0) = \universe{Z} -
  (\conform(\universe{Z},\cB^{-1},\cB^{+1},+1) \cup
  \conform(\universe{Z},\cB^{-1},\cB^{+1},-1))
  \]
\end{itemize}

Intuitively, we want \ActiveLearn\ to focus its queries on the
effective boundary set
$\nonconform(\universe{Z},\cB^{-1},\cB^{+1},0)$. However, for
technical reasons, we need to expand this set to include the
neighborhoods of the boundary sets.
...
$$  \mbox{For } s \in \{-1,0,1\}:\;\; 
  \cC^s_t = \constr{\cC_t}{s}{\bigcup_{i=1}^t \sample{S}_i \cup \sample{U}_t}$$
...
We define restricted sets of specialists to be subsets of $\cB_x$ that
satisfy the following conditions on $\bias(B)$ and $\D(B)$
\[ \restricted{\cB_x}{s,\Delta,\leq p}
=
\left\{
B \in \cB \left| s \bias(B) \geq \Delta \mbox{ and } \D(B) \leq p
\right.\right\}
\]
Instead of $\leq p$, the condition in this definition can be
$<p$ or $>p$ or $\geq p$.

We say that a specialist $B$ is uniformly $(s,\Delta)$-biased if for all
$x \in B$, $s \bias(x) \geq \Delta$
...
As \ActiveLearn\ is at it's core a nearest neighbor algorithm, we
quantify it's performance by consider the difference between the error
rate of the algorithms and the error rate of the Bayes optimal
algorithm. We say that the algorithm is $\epsilon$-competitive if it's
