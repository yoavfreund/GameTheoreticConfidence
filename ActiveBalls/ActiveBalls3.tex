...
mapping from a domain $X$ to a binary label. For the sake of symmetry,
...
{\em Specialists} are a special case of CRC in which the outputs of
the classifier are either in $\{0,+1\}$ (positive specialists) or in
$\{0,-1\}$ (negative specialists).  We use the following notation: the
support of a specialist is a set $A$ on which the classification of
the specialist is non-zero. We denote by $A^+: X \to \{0,+1\}$ the
function the corresponds to the positive specialist and by $A^-$ the
...
We now state our Convergence bound theorm. The proof is given in
Section~(\ref{sec:convergence-bounds}).
...
Using this theorem and given a sample $\sample{S}$ \ActiveLearn
partition the collection $\cB$ into three sets:
\begin{itemize}
\item The probably positive set $\constr{\cB}{+}{\sample{S}}$ contains all $B \in \cB$ such that
  $\ebias(B) \geq \sqrt{\frac{k_0}{k(B)}}$
\item The probably negatively set $\constr{\cB}{-}{\sample{S}}$ contains all $B \in \cB$ such that
  $\ebias(B) \leq -\sqrt{\frac{k_0}{k(B)}}$
\item The undertermined set $\constr{\cB}{0}{\sample{S}}$ contains $B \in \cB$ that are neither in
  $\constr{\cB}{+}{\sample{S}}$ nor in $\constr{\cB}{+}{\sample{S}}$.
\end{itemize}
...
behind \ActiveLearn is to preferably query instances that are close to
the decision boundary. A related intuition is to prefer instances on
which there is disagreement among the best rules.

Specifically, given a partition of $\cB$ into
$\constr{\cB}{+}{\sample{S}},\constr{\cB}{-}{\sample{S}}$ and $\constr{\cB}{0}{\sample{S}}$, we
partition the instances in $\universe{X}$ into {\em interior} and
a {\em boundary} sets. Roughly speaking, interior points are those on
which all of the specialists agree. To this set of points we
add points where all of the specialists with small support
agree. \yoav{I am planning to create a jupyter notebook and
  generate some figures that will be added here, to aid intuition.}

Similarly to Chadhuri and Dasguta~\cite{ChadhuriDasgupta2015}, we
define a partition of a set of points $\universe{Z}$ into effective
interior and boundary sets.
\begin{itemize}
\item {\em  Effective Interior Sets:} There are two interior sets, one for each label.
  Let $y \in \{-1,+1\}$ be that label. Let $\cB^{+1},\cB^{-1}$ be positive and negative conforming
  sets. We define the $y$-interior subset of $\universe{Z}$ as
  \[
  \conform(\universe{Z},\cB^{+1},\cB^{-1},y) =
  \left\{ x \in \universe{Z} \left|
  \begin{array}{c}
    \exists B \in B \in \cB^{y} \mbox{ s.t. } x \in B
    \\ \mbox{ and } \\
    \forall A \in \cB^{-y} \mbox{ s.t. } x \in
    A,\;\;\;
    \exists C \in \cB^{y} \mbox{ s.t. } x\in C
    \mbox{ and } C \subset A
  \end{array}
  \right. \right\}
\]
\item {\em Effective Boundary} contains all points that are not in
  either interior set:
  \[
  \nonconform(\universe{Z},\cB^{-1},\cB^{+1},0) =
  \universe{Z} - (\conform(\universe{Z},\cB^{-1},\cB^{+1},+1) \cup \conform(\universe{Z},\cB^{-1},\cB^{+1},-1))
  \]
\end{itemize}

With these definitions, we can now describe \ActiveLearn\

\newcommand{\Unconf}{\universe{U}}
\newcommand{\all}{\universe{X}}
\noindent
\begin{enumerate}
\item Define the initial boundary to be all of the instances $\Unconf_0=\all$
\item for $t=1,2,\ldots$:
\begin{enumerate}
\item Query $n$ points chosen uniformly at random from $\all$ and $n$
...
\item Partition the specialists in $\cB$ into conforming and non-conforming:
  \[
  \mbox{For } s \in \{-1,0,1\}:\;\; \cB^s_t = \constr{\cB}{s}{\bigcup_{i=1}^t \sample{S}_i}
  \]
\item Define the {\em constrained set of specialists}:
  \[
  \cC_t = \left\{ B \cap \Unconf_{t-1} \left| B \in \cB \right.\right\}
  \]
\item Partition the specialists in $\cC_t$ into conforming and non-conforming:
  \[
  \mbox{For } s \in \{-1,0,1\}:\;\; \cC^s_t =
  \constr{\cC_t}{s}{\bigcup_{i=1}^t \sample{S}_i \cup \sample{U}_t}
  \]
...
\item Define a new effective boundary set:
  \[
  \Unconf_t = \nonconform(\universe{X},\cA_t^{-1},\cA_t^{+1},0)
  \]
\end{enumerate}
\end{enumerate}
...
\begin{definition}
  \begin{itemize}
  \item The $(p,\Delta)$-interior set for $y=+1$ is
      \[
      \interior{\all}{+1}{p,\Delta} =
      \left\{x \in \all \left|
      \forall B \in \cB \mbox{ s.t. } (x \in B \mbox{ and } \D(B) \leq
      p)\;\; \forall z \in B; \bias(z)>\Delta
      \right.\right\}
  \]
  \item The $(p,\Delta)$-interior set for $y=-1$ is
      \[
      ...
      \right.\right\}
  \]
\item The $(p,\Delta)$-boundary set is the complement of the two
  interior sets.
  \[
\interior{\all}{0}{p,\Delta} = \all - (\interior{\all}{+1}{p,\Delta}
\cup \interior{\all}{-1}{p,\Delta})
  \]
\end{itemize}
\end{definition}

The boundary sets are nested. If $p_1\leq p_2$ and $\Delta_1 \leq \Delta_2$, then
$\interior{\all}{0}{p_1,\Delta_1} \subseteq
\interior{\all}{0}{p_2,\Delta_2}$. The Non-conforming sets of
\ActiveLearn\ are related to the boundary sets. It is therefore
natural to characterize the query complexity of a distribution using
the dependence of the probability of the boundary set on $p$ and $\Delta$.

\begin{definition}
  For a given distribution $\D$ and specialist class $\cB$, we define
  the $(\p,\Delta)$-boundary size to be $V(\epsilon,\Delta) =
  \D(\interior{\all}{0}{p,\Delta})$.
\end{definition}

We will show that if $V(\epsilon,\Delta)<\epsilon^\alpha \Delta^\beta$
then \ActiveLearn\ has exponential speedup.
