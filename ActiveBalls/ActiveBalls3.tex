...
Given the sample $\sample{S}$ we partition the collection $\cB$ into
three sets:
\begin{itemize}
\item $\constr{\cB}{+}{S}$ contains all $B \in \cB$ such that
  $\ebias(B) \geq \sqrt{\frac{k_0}{k(B)}}$
\item $\constr{\cB}{-}{S}$ contains all $B \in \cB$ such that
  $\ebias(B) \leq -\sqrt{\frac{k_0}{k(B)}}$
\item $\constr{\cB}{0}{S}$ contains $B \in \cB$ that are neither in
  $\constr{\cB}{+}{S}$ nor in $\constr{\cB}{+}{S}$.
\end{itemize}

\newcommand{\conform}{{\cal C}}
\newcommand{\nonconform}{{\cal N}}

The partition of the specialists in $\cB$ into three sets defines
a partition of the instances in $\universe$ into {\em conforming} and
a {\em non-conforming} sets.
\begin{itemize}
\item {\em Positive Conforming set} $x \in \universe$ is in the
  positive conforming set $\conform_{\constr{\cB}{+}{S}}$ if there
    exists $B \in \constr{\cB}{+}{S}$ such that $x \in B$ and for any
    $A \in \constr{\cB}{-}{S}$ such that $x \in A$ there exists $C \in
    \constr{\cB}{+}{S}$ such that $x\in C$ and $C \subset A$.
\item {\em Negative Conforming set} $x \in \universe$ is in the
  Negative conforming set $\conform_{\constr{\cB}{-}{S}}$ if there
    exists $B \in \constr{\cB}{-}{S}$ such that $x \in B$ and for any
    $A \in \constr{\cB}{+}{S}$ such that $x \in A$ there exists $C \in
    \constr{\cB}{-}{S}$ such that $x\in C$ and $C \subset A$.
\item {\em Nonconforming set} contains all points that do not belong
  to either the positive or the negative conforming sets.
\end{itemize}


At each iteration $t$
\begin{itemize}
\item Query $n$ points independently at random from
  $\nonconform_{t-1}$ and $n$ points from $X$.
\item Define the bias of each $B \in \cB$ using the 
\item
...
\KwIn{A set of unlabeled Examples $X$; 
confidence parameter $\delta$; A batch size $n$}
\For{$t=1,2,\dots$}{ 
