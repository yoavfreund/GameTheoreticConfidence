...
classifier are either in $\{0,1\}$ or in $\{0,-1\}$. For a specialist
$s$ we refer to the set $\{x|s(x)\neq 0\}$ as the {\em support} of $s$.
...
probability of a $+1$ label is close to $1/2$. When learning with
specialists, convergence bounds are used {\em inside the
  learning algorithm}. We use these bounds to compute confidence
intervals on $P(y=+1|s(x) \neq 0)$ - the probability of the label $+1$
conditioned on the instance $x$ being in the support of $s$.

We now state the Convergence bound theorm. The proof is given in the
next section.
...
Let $S = [(x_0,y_0),\ldots,(x_n,y_n)]$ be a sample of $n$ examples
