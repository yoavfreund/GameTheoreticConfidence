...
Given the sample $\sample{S}$ we partition the collecion $\cB$ into
three sets:
\begin{itemize}
\item $\constr{\cB}{+}{S}$ contains all $B \in \cB$
\item $\constr{\cB}{0}{S}$
\item $\constr{\cB}{-}{S}$
\end{itemize}

The core of \ActiveLearn is a rule which partitions the input space
into {\em conforming} and a {\em non-conforming} sets.
