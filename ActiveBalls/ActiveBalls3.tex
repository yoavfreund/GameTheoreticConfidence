...
\section{Using Specialists}

Specialists abstain on potentially large parts of the input
space. Therefor, when predicting with specialists, we use a rule that
combines many specialists. Suppose our transductive sample is $\X$.
Define the {\em weight} of a specialist to be the number of points in
$\X$ on which it predicts (rather than abstain).

Consider two sets of specialists: one in which all of the specialists
have large weight, the other in which each specialist has weight one.
(and each point in $\X$ is covered by a specialist.  Suppose a
randomly chosen subset of the instances in $\X$ are labeled.

Consider first the light specialists. Clearly with these specialists
any labeling of the instances is possible. On the the other hand,
unless the number of labeled examples is much larger than $|\X|$ it is
unlikely that we can identify any specialist as being probably
positive or probably negative (see definitions above).

On the other hand, if the specialists are heavy we are likely to be
able to identify whether they are probably positive or probably
negative because the number of labeled examples that they contain is
large. However, combinations of heavy, low-VC rules have low
representational power.

Intuitively, combinations of light specialists have low bias and
high-variance, while combinations of heavy specialists have high bias
but low variance. Combining specialists of different weights for
different parts of the input space gives us a location-sensitive way
to balance bias and variance.

\section{The active learning algorithm}

Our active learning algorithm $\ActiveLearn$ is based the
bias/variance tradeoff described in the last section.

The algorithm starts by querying a small number of randomly chosen
instances. As the number of labels is small, only heavy specialists
can be placed in the probably positive or probably negative sets.

Given a partition of $\cB$ into $\cB^{+1} (\sample{S}),\cB^{-1}
(\sample{S}), \cB^{0} (\sample{S})$ (a partial determination of the
signs of specialist biases in $\cB$), the algorithm partitions the
instances in $\universe{X}$ into {\em interior} and {\em boundary}
sets, as in \cite{ChadhuriDasgupta2015}. Roughly speaking, interior
points are the ones on which all of the specialists agrees, the
complement, the boundary points are the ones on which there is
disagreement among the specialists.

At this point we want to query the labels for more points so that we
can identify the polarity of specialists with smaller weight.
Intuitively, we would like to query only boundary points. However,
there is a catch, the apparent agreement between the specialists on
the interior points might be a result of the fact that the labels are
too sparse to define the polarity of lighter specialists.

We therefor use a hybrid query selection method: we make half of the
queries on the boundary set and half uniformly from the whole
space. We can think of querying the boundary as exploring the {\em
  ``known unknown''}\footnote{Donald Rumsfeld 2002:
  {\tt http://archive.defense.gov/Transcripts/Transcript.aspx?TranscriptID=2636}}
while querying the interior as looking for the {\em ``unknown
  unknown''}

The fact that we select half of the queries randomly from the whole
space guarantees that the number of queries issued by the algorith is
never mode than twice that of the passive algorithm.  On the other
hand, as we will show, if there are no ``unknown unknowns'' to be
found and if the size of the boundary shrinks with the number of
examples, then $\ActiveLearn$ makes significantly fewer queries than
the passive algorithm.
...
As Theorem~\ref{thm:UCECM} demonstrates, the ability to identify
whether a spe
\fi

\subsection{The details}

We now formalize the intuitive description of $\ActiveLearn$..
