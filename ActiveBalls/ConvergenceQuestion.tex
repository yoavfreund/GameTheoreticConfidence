...
Suppose we have a sample $S$ of size $m$: $(x_1,y_1),\ldots,(x_m,y_m)$.
~\\
~\\

{\bf The non-standard part:}
We want to bound the deviation betweeen the true conditional expectation
$E(Y|h(X)=1)$ and the empirical conditional that is defined to be
$\hat{E}(Y | h(X)=1) \doteq (1/|h(S)|) \sum_{i:h(x_i)=1} y_i$ where $|S(h)|\doteq\sum_{i=1}^m
h(x_i)$. We want the bound on the magnitude of the 
deviation to decrease like  $|h(S)|^{-1/2}$, and increase logarithmically
on $m$.
~\\
~\\

{\bf for a single function this is easy:} Let $H=\{h\}$ be a singleton, then
conditioning on $|h(S)|$ is trivial. As the random variable $|h(S)|$ is
independent of the $y_i$s, we can think of the process of accumulating
the sum $\sum_{i:h(x_i)=1} y_i$ as one where we first choose $|h(S)|$
according to some distribution and then generate $|h(S)|$ examples. We
can therefor use Hoeffding bound to get:
\[
P \left( | E(Y|h(X)=1) -
...
\hat{E}(Y | h(X)=1) | > \frac{2 \ln 1/\delta + d \ln m }{\sqrt{|h(S)|}} \right)
\leq \delta
\]
where $d$ is the VC dimension of $H$ and $m$ is the total number of
examples.
