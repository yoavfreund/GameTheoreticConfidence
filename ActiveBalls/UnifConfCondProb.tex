...
\sqrt{\frac{k_0}{k(B)}},\]
where $k_0 =300\bigl(d\ln(3n) + \ln(1/\delta)\bigr)$, and $k(B) = \sum_{i=1}^n 1[x_i\in B]$.
\end{theorem}
...
once $p(B) \geq d\ln n /n$, whereas the other one becomes non-trivial
...
%Let $\B$ be a family of specialists of VC dimension $d$.
%Set $k_0 = 100\bigl(d\ln(2n) + \ln(10/\frac{\delta}{2})\bigr)$, where
% $1/2\geq \delta>0$,
%and let $S=\bigl((x_i,y_i)\bigr) \sim p^n$.
%Then:
%\[\Pr_{\samp\sim p^n}\left[\exists {B\in\B}:~\lvert
%  \bias_{\samp}(B) -  \bias(B)\rvert \geq \sqrt{\frac{100d\ln n +
%      \ln(1/\delta)}{k(B)} }
%  \mbox{ and } k(B)>k_0
%  \right] \leq \delta,
...
%\bias(B)\rvert \geq \sqrt{\frac{100d\ln n + \ln(1/\delta)}{k_1(B)}}
%\mbox{ and } k_1(B)>k_0
%\text{"}, 
%\]
%and let $E_2$ denote the event:
%\[
%E_2 = \text{"}\exists {B\in\B}:~\lvert \bias_{\samp_1}(B) -  \bias_{\samp_2}(B)\rvert \geq \sqrt{\frac{100d\ln n + \ln(1/\delta)}{k_1(B)}}
...
%is sufficiently large so the $\lvert \bias_{\samp_1}(B) -  \bias(B)\rvert \geq \sqrt{\frac{100d\ln n + \ln(1/\delta)}{k_1(B)} }$.
...
%labels of $O(\ln 1/\epsilon)$ instances.
%
...
%%Setting $k\geq 100\bigl(d\ln(2n) + \ln(10/\delta)\bigr)$ yields
