...
%First, notice that formally speaking the above event is not well defined defined:
%specifically, $p_n(A_i \vert B_i)$ is not defined for $B_i$'s such that $k_i=0$.
%This can easily be fixed, say by restricting the universal quantifier to $(\forall i \text{ s.t. } k_i>0)$.
%It turns out that this is also not enough. 
