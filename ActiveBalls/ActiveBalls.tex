...
\begin{enumerate}
  \item The input space is $R^d$.
    ...
    B_{r,\cc,\sigma}(\x) =
    \begin{cases}
      \sigma & \text{if } \| \cc- \x \| \leq r \\
    0 & \text{otherwise }
    \end{cases}
    \]
    Where $r \geq 0$ is the radius of the ball, $\cc \in R^d$ is the
    center of the ball and $\sigma \in \{-1,+1\}$ is the polarity of the ball.
...
\newcommand{\SepsGamSig}{\SS_{\epsilon,\edge}^{\sigma}}
\newcommand{\SepsGam}{\SS_{\epsilon,\edge}}
\newcommand{\SepsGamMinusSig}{\SS_{\epsilon,\edge}^{-\sigma}}

We denote the set of all balls by $\SS$. For any $\epsilon,\edge>0$
and $\sigma \in \{-1,+1\}$ we define the set of $(\epsilon,\edge,\sigma)$-
good balls $\SepsGamSig \subset \SS$ to be:
\[
\SepsGamSig \doteq \left\{B \in \SS \left| p(B) \geq \epsilon,
\sigma \bias(B) > \edge \right. \right\}
\]

\newcommand{\XepsGamSig}{\X_{\epsilon,\edge}^{\sigma}}
\newcommand{\XepsGam}{\X_{\epsilon,\edge}}

We define the set of $(\epsilon,\edge,\sigma)$-good {\em instances} to be:
\[
\XepsGamSig \doteq \left\{\x \in R^d \left|
\begin{array}{l}
\exists B \in \SepsGamSig \mbox{ s.t. } \vx \in B \mbox{ and } \\
\forall B \in \SepsGamMinusSig \mbox{ s.t. } \vx \in B,\;\;
\exists B' \in \SepsGamSig \mbox{ s.t. } \vx \in B' \mbox{ and } B'
\subset B
\end{array}
\right. \right\}
\]
...
Finally, we relate these definitions back to the linear program. For a given
$\epsilon,\edge$ we define $\SepsGam = \SepsGam^{+1} \cup \SepsGam^{-1}$
...
$\vx$, and consider subset of the spcialists in $\SepsGam$ that
