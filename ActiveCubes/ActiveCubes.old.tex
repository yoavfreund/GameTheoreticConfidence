...
A specialist is a mapping from $\cX$ to $\{0,1\}$ (A positive
specialist) or from $\cX$ to $\{0,-1\}$ (a negative specialist). When a
specialist outputs $0$ we say that it ``abstains''. The specialist
``predicts'' when it does not abstain.

We are interested in classifiers that combine specialists.

The {\em bias} of a specialist is the expected value of the label
given that the specialist is predicting. We define the true and
empirical biases of a specialist in the standard way. The difference
between the empirical and the true biases of a set of specialists can
be bounded uniformly in the standard way, using either the size of the set or
its VC dimension.

Here we use a {\em non-uniform improvement} of the standard uniform
bound.  As stated in the following theorem, the bounds on the
difference between the true and empirical bias of each specialist is
bounded by the number of training examples on which the specialist predicts.
...
\section{Cube Specialists}

We start with a minimalist version of the cube specialists, on which
we can elaborate.

Suppose that the domain is the $d$ dimensional unit cube: $\cX =
[0,1]^d$, fix an integer $k>0$ and define a grid $\cG$ consisting of
the points $(i_1/k,i_2/k,\ldots,i_d/k)$ where $0 \leq i_j \leq k$ are
integers. For ease of notation we drop the $/k$ and index an element
in $\cG$ by $\vec{p} = (i_1,\ldots,i_k)$

The specialists we use are cubes whose corners are grid
points.~\footnote{We could use rectangles, but cubes are simpler,
sufficient, and there are fewer of them than rectangles.} A cube
specialist $C$ is defined by an integer size $s<k$ and a lower-left
corner point $(i_1,\ldots,i_d)$ where $0 \leq i_j \leq k-s$.
The specialist predicts on all inside the cube, in other words,
on all $\vec{x} \in \cX$ such that for all $1 \leq j \leq d$,
$i_j/k \leq x_j \leq (i_j+s)/k$.

We call cubes of size one ``atoms''. The {\em optimal} classifier
predicts on each atom according to the sign of it's bias.
...
\section{Continuation of old paper}
